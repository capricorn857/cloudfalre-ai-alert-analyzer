...
2. 判断告警类型
3. 生成内部 Alert 对象
4. 将 Alert 写入 Cloudflare Queue
5. 尽快向调用方返回 `202 Accepted`

Webhook 接口不执行 Cloudflare GraphQL、LLM 或企业微信调用。
...
- `webhook_received_at` 为 Webhook 通过校验、准备入队时记录的时间
- `query_started_at` 为入队前计算的逻辑查询开始时间，也是本次快照的固定截止时间
- `processing_started_at` 为 Queue Consumer 实际开始处理消息的时间，仅用于日志和耗时观测
- `analysis_window.end` 之后新增的 Security Events 不纳入本次分析
- MVP 不等待未来时间窗口，不因数据持续增长执行二次分析

为缓冲 Cloudflare 数据写入或聚合延迟，系统通过 Cloudflare Queue 延迟投递，在固定时间后开始查询。
...
2026-09-14T09:19:00Z

Logical Query Started At:
...
POST Alert
     │
     ├── Payload Validation
     ├── Fix query_started_at and analysis_window
...
MVP 使用 Cloudflare Queue 解耦 Webhook 接收和告警分析。

MVP 使用同一个 Worker 项目提供 `fetch()` 和 `queue()` Handler，不部署自建 MQ、Redis、Celery 或 Kafka。

Webhook Handler 在入队前按照 `query_started_at = webhook_received_at + settle_seconds` 固定 `query_started_at` 和 `analysis_window`，并将其写入 Queue Message。Queue 默认延迟相同的 `settle_seconds` 后投递消息；Queue Consumer 只校验并复用消息中的固定窗口，后续技术重试不得重新计算或扩大窗口。

正常情况下，每个有效告警只生成一次快照分析和一次通知。Cloudflare Queue 使用至少一次投递语义，极端情况下可能产生重复消费或重复通知。MVP 不引入持久化幂等存储，通知必须展示 `incident_id` 和 `correlation_id`，便于识别重复。
...
所有 Credential 必须通过 Workers Secrets 管理。
...
MVP 使用 Workers 原生 `fetch()` Handler，不引入 FastAPI、Uvicorn、Express 或 Hono。

MVP 使用 Wrangler 直接部署到 Cloudflare Workers，不构建 Docker 镜像，不维护服务器进程。

---

# 26. 配置需求

以下敏感配置必须使用 Workers Secrets：
...
以下非敏感配置通过 Worker Dashboard 的普通 Text 变量管理：
...
业务参数通过 `wrangler.jsonc` 的 JSON var 配置：
...
系统能够成功解析 Payload、将 Alert 写入 Cloudflare Queue，并返回 `202 Accepted`。无效 Payload 不得写入 Queue。
...
`analysis_window.end` 之后新增的数据不纳入本次分析，系统不因数据持续增长执行二次查询。
...
系统能够通过 Wrangler 或 Cloudflare Dashboard 部署为单一 `cloudflare-ai-alert-analyzer` Worker，并复用一套 Queue、vars 和 Secrets。
...
在现有 Cloudflare Queue 基础上增加：

```text
Dead Letter Queue
Cloudflare Workflows
D1 或 Durable Objects 状态记录
```
...
识别 WAF Alert
            ↓
记录 webhook_received_at
            ↓
固定 query_started_at 和 analysis_window
            ↓
写入 Cloudflare Queue
            ↓
延迟固定数据缓冲时间
...
- Credential 不进入代码和日志
- 单次真实告警能够端到端完成分析
- 每次正常消费只进行一轮业务数据快照分析，不持续跟踪后续新增事件
- MVP 接受 Queue 至少一次投递可能造成的极端重复，并通过 `incident_id` 和 `correlation_id` 识别
- 系统通过 Wrangler 部署到 Cloudflare Workers，不依赖 Docker 或自建服务器

满足以上条件后，再进入历史存储、任务系统增强、多告警类型和 Incident Tracking 等后续阶段。
